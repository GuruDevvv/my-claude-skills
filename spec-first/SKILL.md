...
description: >
  ...
  "хочу сделать бота", "нужен сайт для..."), says "new project", "start a project", "plan the project",
  "новый проект", "начинаем", "давай спеку", "запускаем",
  or when existing code has no docs/vision/blueprint.
...
**Q6:** Где это будет работать — телефон, компьютер, Telegram? Нужны ли внешние сервисы? Есть ограничения по бюджету?

**Q7:** Чего точно НЕ будем делать в первой версии?
...
**Q8:** Представь — проект готов. Ты открываешь и проверяешь. Какие 3 вещи сделаешь чтобы убедиться что всё работает?
...
- **Q6:** предложи дефолт: "Для начала — локально. Деплой добавим потом. Ок?"
- **Q8:** предложи 3-4 проверки из контекста проекта, спроси какие подходят
...
## НЕ делаем в v1
## Проверки готовности (превращай ответы Q8 в конкретные проверки с числами — это твоя работа, не пользователя)
...
- Минимум 2 сценария "делаю X → получаю Y" (из Q3/Q8)
...
**Шаг 4. План сборки** — 3-15 задач в правильном порядке. Каждая задача в формате:
```
...
**Шаг 5. Чек-лист проверки** — минимум 5 проверок "Делаю X → Вижу Y" (из Q8 + дополнительные). **Жди ответа.**

**Шаг 6** (если в vision есть AI-поведение): добавь в стек AI-провайдер, задачу на системный промпт, 2+ проверки на AI-поведение.
