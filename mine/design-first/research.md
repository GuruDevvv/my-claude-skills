> **Архив (июнь 2026).** Ресёрч до слепых кругов сентября. Часть советов отсюда устарела (Picsum по умолчанию, HTML-прототипы как основной путь) — действующий путь описан в SKILL.md и references/mockups.md.
...
## Пересмотр 2026-09-25: концепция снаружи ниши → чтение вайба + замер

**Что показала практика за три месяца.** Слой art direction (концепция из-за пределов категории)
решил проблему «шесть шкурок одной страницы», но создал обратную: макеты стали чужими для аудитории.
Три случая с одинаковым рисунком:

- Лендинг вебинара для женщин: девять макетов за три круга забракованы («мрачные, монотонные,
  скучные» → «мало секса» → «смотрю на все это и плачу»). Попал с первого раза типичный вид жанра:
  тёмная страница, большое фото с тёплым контровым светом, свечение. В брифе этот жанр стоял
  анти-референсом.
- Рабочая база знаний: макеты с метафорами (колесо, схема) — «все очень похожи… убрать непрактичное
  творчество». Разные метафоры на одном скелете читаются как одинаковые.
- Сайт репетитора: чистые правильные макеты — «скучно, без иконок, анимаций и фишек, характерных для темы».

**Почему не «сначала попросить референс».** Урок первого случая звучал так, но у заказчика обычно
нет ни референса, ни времени его искать. Значит, картинку из его головы скилл обязан добывать сам —
из аудитории, кодов жанра, чувства и предметов темы. Референс остаётся бонусом, а не входом.

**Что изменилось:** `references/art-direction.md` → `references/vibe.md` (шесть чтений, таблица
«чувство → свойства», расшифровка оценочных слов заказчика, интерактив из темы); варианты теперь —
разные прочтения одного вайба (3–4 «жанр на отлично», 1–2 «жанр + смелый ход», вариант 0).

**Замер перед показом.** Два сборщика из пяти сдали нечитаемый текст (охра по охре), появление по
скроллу оставляло пустые экраны на телефоне, на широком мониторе портрет наезжал на заголовок —
всё это не видно в коде. Добавлен `scripts/check.mjs` (без зависимостей, Chrome по DevTools-протоколу)
и `references/ui-verify.md` — на основе скилла `ui-verify`, присланного пользователем design-first.

**Проверка проверки (25.09):** прогон по 24 макетам реального проекта на 390/1440. Оба принятых
заказчиком финала — чисто; большинство забракованных — с настоящими FAIL (белый текст на розовой
кнопке 2.44, охра на креме 2.8, охра на зелёном 1.42). Негативный контроль (страница с шестью
заложенными дефектами) — пять FAIL из пяти ожидаемых на каждой ширине. Ложные тревоги, найденные и
убранные: переключаемые вкладки, слайды карусели за кадром, зацикленная анимация набора текста,
свёрнутые элементы подсказок.
