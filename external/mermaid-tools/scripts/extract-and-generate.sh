...
if [ ! -f "$CHROME_PATH" ]; then
    # Не Linux/WSL (например, Windows): системного Chrome по этому пути нет.
    # mermaid-cli умеет работать на своём встроенном браузере — просто не навязываем ему путь.
    echo "NOTE: Chrome не найден по $CHROME_PATH — используем браузер, встроенный в mermaid-cli"
    CHROME_PATH=""
fi
